...
struct MoodPlayView: UIViewRepresentable {

    @EnvironmentObject private var orientation: MoodOrientationManager

    let startPoint: URL
    let launch: MoodLaunchStore
    let session: MoodSessionState
    let onReady: () -> Void
...
final class Handler: NSObject, WKNavigationDelegate, WKUIDelegate {
    ...
    private let launch: MoodLaunchStore
    private let orientation: MoodOrientationManager
    ...
    init(
        startPoint: URL,
        launch: MoodLaunchStore,
        orientation: MoodOrientationManager,
